...
import EventBus, { WILDCARD, type EmitContext, type Listener, type Disposer, type EventMap } from '../src/event-bus'
...
describe('EventBus', () => {
    ...
    describe('subscribe and emit', () => {
        it('delivers payload to subscribers', async () => {
            ...
            emitter.subscribe('test', (payload) => { received.push(payload) })
        ...
        it('does not deliver events to unrelated streams', async () => {
            let called = false
            emitter.subscribe('other', () => { called = true })
        ...
        it('handles async subscribers', async () => {
            ...
            emitter.subscribe('test', () => { order.push(2) })
    ...
    describe('unsubscribe', () => {
        it('returns a disposer from subscribe', async () => {
            let count = 0
            const dispose = emitter.subscribe('test', () => { count++ })
        ...
        it('removes a specific callback via unsubscribe', async () => {
            let aCount = 0
            let bCount = 0
            const cbA: Listener = () => { aCount++ }
            const cbB: Listener = () => { bCount++ }
    ...
    describe('once', () => {
        ...
        it('can be manually disposed before firing', async () => {
            let called = false
            const dispose = emitter.once('test', () => { called = true })
    ...
    describe('error isolation', () => {
        it('does not let a bad listener break other listeners', async () => {
            ...
            emitter.subscribe('test', () => { throw new Error('boom') })
            emitter.subscribe('test', () => { secondCalled = true })
        ...
        it('routes errors to custom onError handler', async () => {
            ...
            em.subscribe('test', () => { throw new Error('kaboom') })
            await em.emit('test')
    ...
    describe('clear and dispose', () => {
        it('removes all listeners with clear()', async () => {
            let count = 0
            emitter.subscribe('a', () => { count++ })
            emitter.subscribe('b', () => { count++ })
    ...
    describe('wildcard listener', () => {
        ...
        it('fires alongside stream-specific listeners', async () => {
            const log: string[] = []
            emitter.subscribe('test', () => { log.push('specific') })
            emitter.subscribe(WILDCARD, () => { log.push('wildcard') })
    ...
    describe('namespaced events', () => {
        it('matches combat:* to combat:damage', async () => {
            let called = false
            emitter.subscribe('combat:*', () => { called = true })
        ...
        it('matches exact stream over namespace pattern', async () => {
            const log: string[] = []
            emitter.subscribe('combat:damage', () => { log.push('exact') })
            emitter.subscribe('combat:*', () => { log.push('namespace') })
    ...
    describe('priority ordering', () => {
        it('executes higher priority listeners first', async () => {
            ...
            emitter.subscribe('test', () => { order.push('low') }, { priority: 1 })
            emitter.subscribe('test', () => { order.push('high') }, { priority: 10 })
            emitter.subscribe('test', () => { order.push('mid') }, { priority: 5 })
        ...
        it('defaults to priority 0', async () => {
            ...
            emitter.subscribe('test', () => { order.push('default') })
            emitter.subscribe('test', () => { order.push('explicit-zero') }, { priority: 0 })
            emitter.subscribe('test', () => { order.push('high') }, { priority: 1 })
    ...
    describe('event metadata / EmitContext', () => {
        it('provides timestamp in context', async () => {
            ...
            emitter.subscribe('test', (_payload, c) => { ctx = c })
        ...
        it('provides the stream type in context', async () => {
            let ctx: EmitContext | undefined
            emitter.subscribe('my-event', (_p, c) => { ctx = c })
    ...
    describe('event buffering / replay', () => {
        it('buffers events when no subscribers exist', async () => {
            ...
            em.subscribe('test', (payload) => { received = payload })
        ...
        it('replays multiple buffered events in order', async () => {
            ...
            const received: unknown[] = []
            em.subscribe('test', (payload) => { received.push(payload) })
        ...
        it('does not buffer when disabled (default)', async () => {
            ...
            emitter.subscribe('test', (payload) => { received = payload })
        ...
        it('clears buffer after replay', async () => {
            ...
            const first: unknown[] = []
            em.subscribe('test', (payload) => { first.push(payload) })

            const second: unknown[] = []
            em.subscribe('test', (payload) => { second.push(payload) })
    ...
    describe('emitDynamic', () => {
        ...
        it('works with wildcard listeners', async () => {
            const types: string[] = []
            emitter.subscribe(WILDCARD, (_p, ctx) => { types.push(ctx.type) })
    ...
    describe('typed EventBus', () => {
        ...
        it('delivers complex typed payloads', async () => {
            ...
            await typedBus.emit('combat:damage', { target: 1, amount: 50 })

            expect(received).toEqual({ target: 1, amount: 50 })
        })
        ...
        it('supports wildcard on typed bus with unknown payload', async () => {
            ...
            await typedBus.emit('combat:damage', { target: 1, amount: 10 })
        ...
        it('supports namespace patterns on typed bus', async () => {
            ...
            await typedBus.emit('combat:damage', { target: 1, amount: 25 })
            await typedBus.emit('inventory:add', { entityId: 1, item: 'sword' })

            expect(events).toHaveLength(1)
            expect(events[0]).toEqual({ target: 1, amount: 25 })
        })

        it('supports emitDynamic for untyped engine dispatch', async () => {
            const received: unknown[] = []
            typedBus.subscribe('narrate', (payload) => { received.push(payload) })
