...
export function splitRawCommands(rawCommand: string): SplitCommand[] {
  const prefixRegex = /^(?:would i be able to|could i|can i|is it possible to|may i|am i able to)\s*/i;
  const splitRegex =
    /\b(?:(?:(?:\.|\?|!|;|\?!|!\?)(?:\s+|$|\b)(?:(?:\s*,\s*)?(?:\s*(?:\band\b|&)\s*)?then)?)|(?:\s*(?:,\s*)?(?:\s*(?:\band\b|&)\s*)?then))/i;
  ...
  return commands.map((cmd) => {
    ...
    previousCommandIsDryRun =
      previousCommandIsDryRun || newCmd.length !== cmd.length;
